...
After running `bash install.sh`, URLShine and all tools are automatically installed. To use them from **anywhere** on your machine:

### **Step 1: The installer handles most of it**
```bash
bash install.sh  # Automatically:
                 # - Compiles URLShine
                 # - Installs to /usr/local/bin (accessible globally)
                 # - Installs Go tools to ~/go/bin
                 # - Installs Python tools to ~/.local/bin
```

### **Step 2: Update your shell configuration (if needed)**

If you see "command not found" errors after installation, add Go binaries to PATH:
...
```bash
# Test URLShine (should work from any directory)
urlshine doctor
...
### **Troubleshooting: Tools not found after installation**

**If `urlshine doctor` works but other tools don't:**
...
# Bash: echo 'export PATH="$PATH:$HOME/go/bin:$HOME/.local/bin"' >> ~/.bashrc
# Zsh:  echo 'export PATH="$PATH:$HOME/go/bin:$HOME/.local/bin"' >> ~/.zshrc
